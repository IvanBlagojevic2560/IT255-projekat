    ...
    return {
        ...
        execute: function() {
            ForumComponent = (function () {
                function ForumComponent(builder, http, router) {
                    var _this = this;
                    this.http = http;
                    this.router = router;
                    ...
                    if (localStorage.getItem('token')) {
                        http.get('http://localhost/phpProjekat/getUserByToken.php?token=' + this.token, { headers: headers })
                        ...
                        http.get('http://localhost/phpProjekat/getAdminByToken.php?token2=' + this.token2, { headers: headers })
                            .map(function (res) { return res.json(); })
                            .subscribe(function (username) {
                            _this.komentarForm.controls['username'].updateValue(username.username.username);
                        }, function (err) { return console.log(JSON.stringify(err)); });
                ...
                ForumComponent.prototype.onAddComment = function () {
                    var _this = this;
                    var data = "komentar=" + this.komentarForm.value.komentar + "&user=" + this.komentarForm.value.username;
                    var headers = new http_1.Headers();
                    headers.append('Content-Type', 'application/x-www-form-urlencoded');
                    this.http.post('http://localhost/phpProjekat/dodajKomentar.php', data, { headers: headers })
